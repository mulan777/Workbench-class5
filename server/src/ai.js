const AI_BASE_URL = process.env.ZT_AI_BASE_URL || 'http://45.207.215.144:3000'
const AI_KEY = process.env.ZT_AI_KEY || ''
const PRIMARY_MODEL = process.env.ZT_AI_PRIMARY || '[anti]gemini-3.7-flash'
const FALLBACK_MODEL = process.env.ZT_AI_FALLBACK || '[anti]gemini-3.1-pro-high'
...
function imagePrompt(paths) { return { messages:[{role:'user',content:[{type:'text',text:`请分析这${paths.length}张幼儿园签到板图片。综合识别能看清的幼儿姓名、结伴关系、日期或备注；多张图片中重复内容请合并。看不清的内容明确标注“无法确认”，不要猜测。请用中文分点输出，最后给出适合教师核对的简短提示。`},...paths.map(url=>({type:'image_url',image_url:{url}}))]}],temperature:0.1,max_tokens:1800,stream:true} }
async function analyze(app,reply,paths,createdBy,recordId) {
  ...
  const result=await choose(imagePrompt(urls)); copyHeaders(reply,result)
...
export function registerAiRoutes(app) {
  app.post('/v1/chat/completions',{preHandler:[app.auth]},async(req,reply)=>{ if(!AI_KEY)return reply.code(503).send({error:{message:'AI接口未配置密钥'}}); const res=await choose(req.body||{}); copyHeaders(reply,res); if(!res.ok)return reply.send(await res.text()); return pipe(reply,res) })
  ...
  app.post('/api/checkin/analyze-existing',{preHandler:[app.auth]},async(req,reply)=>{ if(!AI_KEY)return reply.code(503).send({error:'AI接口未配置密钥'}); const paths=[...new Set((req.body?.paths||[]).map(String))].filter(p=>/^\/uploads\/[\w.-]+$/.test(p)).slice(0,12); if(!paths.length)return reply.code(400).send({error:'请选择至少一张已上传图片'}); return analyze(app,reply,paths,Number(req.user.sub)||1) })
  app.post('/api/checkin/analyze',{preHandler:[app.auth]},async(req,reply)=>{ if(!AI_KEY)return reply.code(503).send({error:'AI接口未配置密钥'}); const f=await req.file(); if(!f)return reply.code(400).send({error:'未收到签到图片'}); if(!/^image\/(png|jpe?g|webp|gif)$/i.test(f.mimetype||''))return reply.code(400).send({error:'仅支持 PNG/JPG/WebP/GIF 图片'}); const buf=await f.toBuffer(); if(buf.length>15*1024*1024)return reply.code(400).send({error:'图片不能超过15MB'}); const media=`data:${f.mimetype};base64,${buf.toString('base64')}`; return analyze(app,reply,[media],Number(req.user.sub)||1) })
}
